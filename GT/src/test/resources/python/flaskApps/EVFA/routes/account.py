...
def get_personal_notes(user_id: int):
    with Session() as session:
        personal_notes = session.query(Note).filter(Note.user_id == user_id).all()
...
def add_image():
    ...
    if not form.validate():
        ...
        with Session() as session:
            current_user.profile_image = get_base64_image_blob(form.url.data).encode()
...
def update_account():
    ...
    if not form.validate():
        ...
        with Session() as session:
            filtered_values = {key: value for key, value in form.data.items() if value is not None and key != 'password'}
            current_user.__dict__.update(filtered_values)

            new_password = form.password.data
            was_password_changed = new_password is not None and new_password != filtered_values.get('password_control')

            if was_password_changed:
                current_user.password = hashpw(new_password.encode(), gensalt()).decode()
