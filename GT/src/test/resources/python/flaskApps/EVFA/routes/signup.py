...
def validate_token(code: str, session: Session) -> Union[str, None]:
    try:
        result = session.execute(text(f"""SELECT id, code FROM {RegistrationCode.__tablename__} WHERE code = '{code}'""")).first()
...
def do_signup():
    ...
    if not form.validate():
        ...
        with Session() as session:
            user_already_exists = session.query(session.query(User).where(User.email == form.email.data).exists()).scalar()
            ...
            user = User(form.email.data, hashpw(form.password.data.encode('utf-8'), gensalt()).decode())
