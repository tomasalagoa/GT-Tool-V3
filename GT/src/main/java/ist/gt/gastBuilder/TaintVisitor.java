    ...
    public void visit(FunctionCall functionCall) {
        ...
        if (functionCall.isSuper()) {
            ...
            if (foundFunctions.size() == 1) {
                processFunction(foundFunctions.getFirst().getFunction(), functionCall,
                        foundFunctions.getFirst().getFile(), foundFunctions.getFirst().getClazz());
            }
    ...
    private void processFunction(Function function, FunctionCall functionCall, File file, Class clazz) {
        for (int i = 0; i < function.getParameters().size() && i < functionCall.getMembers().size(); i++) {
            ...
            if (parameter.getType() != null) {
                ...
                } else if (parameter.getMembers().size() == 1 && parameter.getMembers().getFirst().getLambdaFunc() != null) {
                    function.getParameters().getElement(i).setLambdaFunc(parameter.getMembers().getFirst().getLambdaFunc());
                } else {
        ...
        boolean giveTempClassReferenceForFunction = !this.methodCallReferencePropagation.isEmpty()
                && this.methodCallReferencePropagation.getLast().equals(function.getName());
        ...
        if (((!this.tempThis.isEmpty() && this.tempThis.getLast() != null)
                || (!this.noClassReferencePropagation.isEmpty() &&
                this.noClassReferencePropagation.getLast()))
                && giveTempClassReferenceForFunction) {
            this.currentThis = this.tempThis.removeLast();
            this.thisReplacements.add(this.currentThis);
            this.methodCallReferencePropagation.removeLast();
        }
        ...
        if (functionCall.isConstructor() && functionCall.getHiddenThis().getClassReference() != null) {
            ...
            if (giveTempClassReferenceForFunction) {
                ...
                taintVisitor.setDefaultReferencePropagation(this.noClassReferencePropagation.getLast());
            } else {
        ...
        if ((this.currentThis != null || (!this.noClassReferencePropagation.isEmpty() &&
            ...
            if (!this.noClassReferencePropagation.isEmpty()) {
                this.noClassReferencePropagation.removeLast();
            }
            this.thisReplacements.removeLast();
            if (!this.thisReplacements.isEmpty()) {
                this.currentThis = this.thisReplacements.getLast();
            }
        }
    }
...
//            CommonTools.throwAny(new Exception("Found vulnerability " + functionCall.getLine()));
    ...
    public void visit(Expression expression) {
        ...
        if (expression.getMembers().size() == 1 && expression.getMembers().getFirst().getTrackedValue() != null) {
            ...
            expression.setType(expression.getMembers().getFirst().getType());
            if (expression.getMembers().getFirst().getClassReference() != null) {
                expression.setClassReference(expression.getMembers().getFirst().getClassReference());
                expression.setSelectedAttribute(expression.getMembers().getFirst().getSelectedAttribute());
            } else if (expression.getMembers().getFirst().getTrackedValue() == null &&
                    expression.getMembers().getFirst().getLambdaFunc() == null &&
                    !expression.getMembers().getFirst().isCollection()) {
    ...
    private List<FileAndFunction> getFunctionsForType(String type, FunctionCall funcCall) {
        var classesForType = files.stream().filter(iterFile -> iterFile.getClasses().containsKey(type))
                .map(iterFile -> new FileAndClass(iterFile, iterFile.getClasses().get(type))).toList();
    ...
    /**
     * @param assignment This function is used for value tracking of the left side of the
    ...
    public void trackReturnValueForMethodCall(MethodCallExpression methodCall) {
        if (methodCall.getMembers().size() == 1 && methodCall.getMembers().getFirst() instanceof FunctionCall functionCall) {
            ...
            if (file.getClasses().containsKey(constructorCall.getFunctionName())) {
                ...
                if (clazz.getConstructors().size() == 1) {
                    return new FileAndFunction(file,
                            clazz.getConstructors().getFirst(), clazz);
