...
public class GastBuilder {
    ...
    /**
     * This function creates a new Literal based on the context generated by the parser of a specific language.
     * To implement language-specific constructs, the developer only needs to add the specific conditional related to
     * their construct.
     * The actual creation of the Literal is deferred to the later addLiteral function.
     *
     ...
     */
    public Literal addLiteral(ParserRuleContext ctx, LiteralOptions opts, String ruleType) {
        String type = "null";
        ...
        var constant = new Literal(ctx, value, type);
    ...
    public void normalIncrementDecrementExpression(ParserRuleContext ctx, String operator, String condType) {
        ...
        if (statements.peek() instanceof Expression) {
            ...
            Expression expression = new Expression(ctx);
            Assignment assignment = new Assignment(ctx);
            Literal literal = new Literal(ctx, "1", "int");
            ...
            expression.getMembers().add(literal);
